-- Drop the vulnerable public table that exposes sensitive trading data
DROP TABLE IF EXISTS public.exness_sessions_public CASCADE;
...
SELECT log_security_event(
  ...
  NULL,
  jsonb_build_object(
    'action', 'dropped_public_table',
    'reason', 'Removed publicly accessible table containing sensitive trading data',
    'timestamp', now()
  )
);
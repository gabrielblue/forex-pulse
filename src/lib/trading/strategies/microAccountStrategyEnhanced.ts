/**
 * Enhanced Micro Account Growth Strategy
 * Multi-timeframe alignment + candle behavior analysis
 * For $8 accounts to grow to $100 with minimal risk
 */

import { TradingSignal } from '../signalProcessor';
import { getPipValue } from '../tradingUtils';

export interface MicroAccountSignalEnhanced extends TradingSignal {
  atr: number;
  ...
  candleRejection: boolean;
  liquiditySweep: boolean;
}

export class MicroAccountStrategyEnhanced {
  /**
  ...
  private hasCandleRejection(prices: number[], index: number): boolean {
    // Simplified check - in real implementation would need OHLC data
    // Look for price rejecting at liquidity levels
   ...
   */
  private getHigherTimeframeBias(prices: number[]): 'BULLISH' | 'BEARISH' | 'NEUTRAL' {
    // Simplified: use EMA relationship on higher timeframe
  ...
  /**
   * Check if trading session is active (London/NY)
   */
  ...
  /**
   * Main enhanced strategy with multi-timeframe alignment
   */
  async executeEnhancedMicroStrategy(
    symbol: string,
    prices: number[],
    currentPrice: number,
    htfPrices: number[], // Higher timeframe prices (e.g., H1 for M15 strategy)
    accountBalance: number = 8
  ): Promise<MicroAccountSignalEnhanced | null> {
    
    if (prices.length < 50 || htfPrices.length < 20) return null;
    ...
    let candleRejection = false;
    let liquiditySweep = false;
    
    // Market structure analysis with multi-timeframe alignment
    if (recentHighs.length >= 2 && recentLows.length >= 2) {
      ...
      if (lastHigh > prevHigh && lastLow > prevLow) {
        ...
        if (htfBias === 'BULLISH') {
          direction = 'BUY';
          confidence = 75;
          reasoning = 'Bullish market structure + HTF alignment: higher highs and higher lows';
          ...
          liquiditySweep = this.hasLiquiditySweep(prices, liquidity.highs);
          if (liquiditySweep) {
            confidence += 5;
      ...
      // Bearish structure: lower highs and lower lows
        ...
        if (htfBias === 'BEARISH') {
          direction = 'SELL';
          confidence = 75;
          reasoning = 'Bearish market structure + HTF alignment: lower highs and lower lows';
          isHighProbability = true;
          ...
          // Check for liquidity sweep
          liquiditySweep = this.hasLiquiditySweep(prices, liquidity.lows);
          if (liquiditySweep) {
            confidence += 5;
            reasoning += ' | Liquidity sweep detected';
          }
    ...
    // Only take high-probability trades with multi-timeframe alignment
    if (confidence < 70 || !isHighProbability) {
      return null;
    }
    ...
    return {
      id: `micro_enhanced_${Date.now()}_${symbol}`,
      symbol,
      type: direction,
      ...
      source: 'Enhanced Micro Account Strategy',
      strategyName: 'Multi-TF Market Structure + Liquidity',
      atr,
      volatilityScore,
      trendStrength,
      momentumScore,
...
export const microAccountStrategyEnhanced = new MicroAccountStrategyEnhanced();
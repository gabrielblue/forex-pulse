...
export interface TradingSignal {
	...
	reasoning: string;
	source: string;
	// Optional regime metadata for dynamic sizing
	regimeExpectancy?: number; // 0-100
	regime?: string;
}
	...
	private async processSignal(signalData: any): Promise<void> {
		try {
			const signal: TradingSignal = {
				...
				regimeExpectancy: signalData.regime_expectancy ? parseFloat(signalData.regime_expectancy.toString()) : undefined,
				regime: signalData.market_regime || undefined
			};
	...
	private async executeSignal(signal: TradingSignal): Promise<void> {
		try {
			...
			const orderRequest: OrderRequest = {
				...
				regimeExpectancy: signal.regimeExpectancy,

...
class TradingBot {
    constructor() {
        ...
        this.isPersistent = true; // Bot continues running across page navigation
        this.healthCheckInterval = null;
        this.lastHealthCheck = new Date();
        // Enhanced capabilities
        this.marketAnalysisActive = false;
        this.sessionAlertsEnabled = true;
        this.chartAnalysisEnabled = true;
        this.eliteStrategiesEnabled = true;
    }
    async initialize() {
        try {
        ...
        try {
            ...
            if (botSettings) {
                this.configuration = {
                    ...
                    tradingHours: botSettings.trading_hours || {
                        start: '00:00',
                        end: '23:59',
                        timezone: 'UTC'
                    },
    ...
    async connectToExness(credentials) {
        try {
    ...
    async updatePerformanceMetrics() {
        try {
            ...
            if (tradingStats) {
                ...
                this.status.weeklyPnL = weeklyTrades?.reduce((sum, trade) => sum + (parseFloat(trade.profit?.toString() || '0')), 0) || 0;
            }
        }
        catch (error) {
    ...
    async performAdvancedChartAnalysis(symbol, timeframe) {
        if (!this.chartAnalysisEnabled)
            return null;
        try {
    ...
    async executeTradeWithAnalysis(symbol, type, volume) {
        try {
            ...
            const analysisSupportsTradeDirection = (type === "BUY" && chartAnalysis.recommendation.action.includes("BUY")) ||
    ...
    async generateTestSignal() {
        ...
        const randomPair = this.configuration.enabledPairs[Math.floor(Math.random() * this.configuration.enabledPairs.length)];
    ...
    async executeManualTrade(symbol, type, volume) {
        try {
            if (!this.status.isConnected) {
    ...
    getMarketNotes(symbol, limit = 20) {
    ...
    async toggleEnhancedFeature(feature, enabled) {
        switch (feature) {
            case 'marketAnalysis':
